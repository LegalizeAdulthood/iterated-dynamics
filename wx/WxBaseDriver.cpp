...
void WxBaseDriver::put_string(int row, int col, int attr, const char *msg)
{
    if (-1 != row)
    {
        g_text_row = row;
...
void WxBaseDriver::unstack_screen()
{
...
void WxBaseDriver::discard_screen()
{
    if (!m_saved_screens.empty())
    {
        // unstack
...
int WxBaseDriver::init_fm()
{
    return 0;
}

void WxBaseDriver::buzzer(Buzzer kind)
{
    wxBell();
}

bool WxBaseDriver::sound_on(int frequency)
{
    return false;
}

void WxBaseDriver::sound_off()
{
}

void WxBaseDriver::mute()
{
}
...
void WxBaseDriver::put_char_attr(int char_attr)
{
    wxGetApp().put_char_attr(g_text_row, g_text_col, char_attr);
}

void WxBaseDriver::delay(int ms)
{
    wxGetApp().pump_messages(false);
    std::this_thread::sleep_for(std::chrono::milliseconds(ms));
}

void WxBaseDriver::set_keyboard_timeout(int ms)
{
...
bool WxBaseDriver::get_filename(
