...
void CPertEngine::pert_functions(Complex *x_ref, Complex *delta_sub_n, Complex *delta_sub_0)

    {
    double  dnr, dni;
    double r = x_ref->x;
    double i = x_ref->y;
    double a = delta_sub_n->x;
    double b = delta_sub_n->y;
    double a2 = a * a;
    double b2 = b * b;
    double a0 = delta_sub_0->x;
    double b0 = delta_sub_0->y;
    double ar = a * r;
    double ib = i * b;
    double ab;
    double x = r;
    ...
    switch (m_subtype)
	    ...
	    case 1:								// Power
	        ...
	        for (int i = 0; i < m_power; i++)
		        {
		        sum += zp * (double)m_pascal_array[i];
                sum *= *delta_sub_n;
		        zp *= *x_ref;
		        }
	        *delta_sub_n = sum;
	        *delta_sub_n += *delta_sub_0;
	        ...
	        break;
	    case 2:								// Burning Ship 
	        delta_sub_n->x = 2.0 * a * r + a2 - 2.0 * b * i - b2;
	        delta_sub_n->y = diff_abs(r * i, r * b + i * a + a * b) * 2;
	        *delta_sub_n += *delta_sub_0;
	        break;
...
void CPertEngine::ref_functions(Complex *centre, Complex *Z, Complex *z_times_2)
    {
	...
	case 0:							// optimise for Mandelbrot by taking out as many steps as possible
        ...
        Z->x = temp_real + centre->x;
        real_imag = z_times_2->x * Z->y;
        Z->y = real_imag + centre->y;
	    break;
	case 1:
	    if (m_power == 3)
	...
	case 6:							// Celtic
        ...
        real_imag = z_times_2->x * Z->y;
        Z->y = real_imag + centre->y;
        Z->x = fabs(sqr_real - sqr_imag) + centre->x;
        break;
	case 7:							// Cubic Celtic
        z = Z->CPolynomial(3);
	...
	case 10:						// Mandelbar (Tricorn)
        sqr_real = sqr(Z->x);
        sqr_imag = sqr(Z->y);
        real_imag = Z->x * z_times_2->y;
        Z->x = sqr_real - sqr_imag + centre->x;
        Z->y = -real_imag + centre->y;
	    break;
	case 11:						// Mandelbar (power)
        z = Z->CPolynomial(m_power);
