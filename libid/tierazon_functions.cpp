...
static std::complex<double> g_z, g_z2, g_q;
...
int init_tierazon_functions(int subtype, std::complex<double> *z, std::complex<double> *q)
    {
    ...
    switch (g_fractal_type)
        ...
        case fractal_type::NEWTONVARIATION:         // Newton variation, z=((z*z*z-z-1)/(3*z*z-1)-z)*c
            if (!g_julia_flag)                      // let's worry about Julai sets later
		        {
                z->real(q->real() + g_params[0]);
		        z->imag(q->imag() + g_params[1]);
		        }
	        break;
...
*/
        ...
        case fractal_type::NEWTONFLOWER:            // Newton/Mandel, Newton Flower
            if (!g_julia_flag)
		        {
                z->real(-q->real() * (double) (g_degree - 2) / (double) g_degree + g_params[0]);
                z->imag(-q->imag() * (double) (g_degree - 2) / (double) g_degree + g_params[1]);
		        }
	        break;
        case fractal_type::NEWTONAPPLE:             // More Newton Msets, 4th order Newton's apple
            if (!g_julia_flag)
                *z = *q * ((double) g_degree);
            *z = complex_fn.complex_invert(*z);
            z->real(z->real() + g_params[0]);
	        z->imag(z->imag() + g_params[1]);
	        break;
	    ...
	    case fractal_type::NEWTONCROSS:             // More Newton Msets, 5th order Newton Mset
            if (!g_julia_flag)
		        *z = *q * 2.0;
            *z = complex_fn.complex_invert(*z);
            z->real(g_params[0] - z->real());
	        z->imag(g_params[1] - z->imag());
	        break;
        ...
        case fractal_type::QUARTET:
            ...
            switch (subtype)
                ...
                case 3:                             // Quartets, t=0; z1=z; z=z*z*z*z-t*t*t*t+c; t=z1
                    g_z2.real(g_params[0]); // t = 0;
                    g_z2.imag(g_params[1]);
                    if (!g_julia_flag)
                        *z = 0;
                    break;
...
int run_tierazon_functions(int subtype, std::complex<double> *z, std::complex<double> *q)
    {
    std::complex<double> a, b, c, zd, z1, zt;
    double d;
    CComplexFn complex_fn;
    ...
    case fractal_type::NEWTONNOVA:                  // Nova, init: z=1; iterate: z=z-((z*z*z-1)/(3*z*z))+c
	        z1 = *z;
	        g_z2 = complex_fn.complex_square(*z);
	        *z = *z - ((*z*g_z2 - 1.0) / (g_z2 * 3.0)) + *q;
	        zd = *z - z1;
	        d = complex_fn.sum_squared(zd);
	        return (d < MINSIZE);

	    case fractal_type::TALIS:                   // Talis, z=((z^degreez)/(m+z^(degree-1))+c 
            {
            double m = g_params[3];
            a = complex_fn.complex_polynomial(*z, g_degree - 1);
            *z = (a * *z) / (m + a) + *q;
            g_new_z.x = z->real();
            g_new_z.y = z->imag();
	    ...
	    case fractal_type::NEWTONVARIATION:         // Newton variation, z=((z*z*z-z-1)/(3*z*z-1)-z)*c
	        z1 = *z;
	        g_z2 = complex_fn.complex_square(*z);
	        *z = ((*z*g_z2 - *z - 1.0) / (g_z2 * 3.0 - 1.0) - *z) * *q;  
	        zd = *z - z1;
	        d = complex_fn.sum_squared(zd);
	        return (d < MINSIZE);
	    ...
	    case fractal_type::NEWTONPOLYGON:           // Newton/Mandel, Newton Polygon
	        {
	        std::complex<double>	fn, f1n, f2n, a, b;

	        z1 = *z;
            f2n = complex_fn.complex_polynomial(*z, g_degree - 2);     // z^(deg - 2) - second derivative power
	    ...
	    case fractal_type::NEWTONAPPLE:             // More Newton Msets, 4th order Newton's apple
	        {
	        double	s, t, u;
	        std::complex<double>	fn, f1n, f2n, f3n, e, f;

	        z1 = *z;
	        f3n = complex_fn.complex_polynomial(*z, g_degree - 4);		// z^(deg - 4) - third derivative power
	    ...
	    case fractal_type::NEWTONCROSS:             // More Newton Msets, 5th order Newton Mset
	        {
	        double	s, t;
	        std::complex<double>	fn, f1n, f2n;

	        z1 = *z;
	        f2n = complex_fn.complex_polynomial(*z, g_degree - 3);		// z^(deg - 3) - second derivative power
        ...
        case fractal_type::QUARTET:
            ...
            switch (subtype)
                ...
                case 0:                             // Quartets, z2=2; z=z*z*z*z+z2+c; z2=z1
                    z1 = *z;
                    *z = complex_fn.complex_polynomial(*z, g_degree) + g_z2 + *q;
                    g_z2 = z1;
                    zd = *z - z1;
                    d = complex_fn.sum_squared(zd);
                ...
                case 1:                             // Quartets, z2=z; z=z*z*z*z+5*z2*c; z2=z1
                    z1 = *z;
                    *z = complex_fn.complex_polynomial(*z, g_degree) + g_z2 * *q * 5.0;
                    g_z2 = z1;
                    zd = *z - z1;
                    d = complex_fn.sum_squared(zd);
                    return (d < MINSIZE || d > MAXSIZE);
	            ...
	            case 2:					            // Quartets, t=0; z1=z; z=z*z*z-t*t*t+c; z=z1
	                z1 = *z;
	                a = complex_fn.complex_polynomial(*z, g_degree) + *q;
	                b = complex_fn.complex_polynomial(g_z2, g_degree);	// global z2 used for t
	                *z = a - b;
	                g_z2 = z1;
	                                                //    z=z*z*z-t*t*t+c;
	                                                //    t=z1;
	            ...
	            case 4:					            // Quartets, z2=z; z=(z^4)+c; c=z2
	                ...
	                g_z2 = *z;
	                                                //  z = z*z*z*z+q;
	                *z = complex_fn.complex_polynomial(*z, g_degree) + *q;
	                *q = g_z2;
	                zd = *z - z1;
	                d = complex_fn.sum_squared(zd);
	                return (d < MINSIZE || d > MAXSIZE);
	            ...
	            case 6:					            // Quartets, z1=z; z=z*z*z*z+z2/2+c;; z2=z1
	                z1 = *z;
	                                                //    z=z*z*z*z+z2/2+q;
	                *z = complex_fn.complex_polynomial(*z, g_degree) + g_z2 / 2.0 + *q;
	                g_z2 = z1;
	                zd = *z - z1;
	                d = complex_fn.sum_squared(zd);
	                return (d < MINSIZE || d > MAXSIZE);
