...
class CPertEngine 
    ...
    int     reference_zoom_point(Complex *centre, int maxIteration, char *status_bar_info);
	void	load_pascal(long PascalArray[], int n);
    double  diff_abs(const double c, const double d);
	void	pert_functions(Complex *XRef, Complex *DeltaSubN, Complex *DeltaSub0);
    void    ref_functions_bf(BFComplex *centre, BFComplex *Z, BFComplex *ZTimes2);
    void    ref_functions(Complex *centre, Complex *Z, Complex *ZTimes2);
