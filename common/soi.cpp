...
scan:
    ...
    tzr4 = GET_REAL(state.cr2, ci2);
    tzi4 = GET_IMAG(state.cr2, ci2);
    ...
    while (1)
    {
        ...
        // iterate test point
        tzi1 = (tzi1 + tzi1)*tzr1 + state.ci1;
        tzr1 = trq1 - tiq1 + state.cr1;
        trq1 = tzr1*tzr1;
        tiq1 = tzi1*tzi1;

        tzi2 = (tzi2 + tzi2)*tzr2 + state.ci1;
        tzr2 = trq2 - tiq2 + state.cr2;
        trq2 = tzr2*tzr2;
        tiq2 = tzi2*tzi2;
        ...
        state.l1 = GET_REAL(state.cr1, ci2);
        state.l1 = (tzr3 == 0.0)?
           (state.l1 == 0.0)?1.0:1000.0:
           state.l1/tzr3;
        if (FABS(1.0 - state.l1) > twidth)
        ...
        state.l2 = GET_IMAG(state.cr1, ci2);
        state.l2 = (tzi3 == 0.0)?
           (state.l2 == 0.0)?1.0:1000.0:
           state.l2/tzi3;
        if (FABS(1.0 - state.l2) > twidth)
        ...
        state.l1 = GET_REAL(state.cr2, ci2);
        state.l1 = (tzr4 == 0.0)?
           (state.l1 == 0.0)?1.0:1000.0:
           state.l1/tzr4;
        if (FABS(1.0 - state.l1) > twidth)
        ...
        state.l2 = GET_IMAG(state.cr2, ci2);
        state.l2 = (tzi4 == 0.0)?
           (state.l2 == 0.0)?1.0:1000.0:
           state.l2/tzi4;
        if (FABS(1.0 - state.l2) > twidth)
