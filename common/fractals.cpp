...
#define LONGEXPBAILOUT()  \
    if (labs(g_l_old_z.y) >= (1000L << bitshift)) \
        return 1;\
    if (labs(g_l_old_z.x) >=    (8L << bitshift)) \
        return 1;
...
{
    if (fabs(modulus(g_new_z)-modulus(old)) < parm2.x)
    {
        return 1;
    }
    old = g_new_z;
...
int NewtonFractal2()
{
    ...
    {
        t2 = 1.0 / t2;
        old.x = t2 * (g_new_z.x * tmp.x + g_new_z.y * tmp.y);
        old.y = t2 * (g_new_z.y * tmp.x - g_new_z.x * tmp.y);
    }
    return 0;
}
...
Barnsley1Fractal()
{
    ...
    oldxinitx   = multiply(g_l_old_z.x, g_long_param->x, bitshift);
    oldyinity   = multiply(g_l_old_z.y, g_long_param->y, bitshift);
    oldxinity   = multiply(g_l_old_z.x, g_long_param->y, bitshift);
    oldyinitx   = multiply(g_l_old_z.y, g_long_param->x, bitshift);
    // orbit calculation
    if (g_l_old_z.x >= 0)
    {
...
JuliafpFractal()
{
    ...
    g_new_z.y = 2.0 * old.x * old.y + g_float_param->y;
    return floatbailout();
}
...
LambdaFractal()
{
    ...
    g_l_new_z.x = multiply(g_long_param->x, g_l_temp_sqr_x, bitshift)
             - multiply(g_long_param->y, g_l_temp_sqr_y, bitshift);
    g_l_new_z.y = multiply(g_long_param->x, g_l_temp_sqr_y, bitshift)
...
SierpinskiFPFractal()
{
    ...
    g_new_z.x = old.x + old.x;
    g_new_z.y = old.y + old.y;
    if (old.y > .5)
    {
...
LambdaexponentFractal()
{
    ...
    //multiply by lamda
    g_new_z.x = g_float_param->x*tmp.x - g_float_param->y*tmp.y;
    g_new_z.y = g_float_param->y*tmp.x + g_float_param->x*tmp.y;
    old = g_new_z;
    return 0;
}
...
LongLambdaexponentFractal()
{
    ...
    g_l_new_z.x  = multiply(g_long_param->x, g_l_temp.x, bitshift)
              - multiply(g_long_param->y, g_l_temp.y, bitshift);
    g_l_new_z.y  = multiply(g_long_param->x, g_l_temp.y, bitshift)
...
LongTrigPlusExponentFractal()
{
    ...
    g_l_new_z.x += multiply(longtmp,    lcosy,   bitshift) + g_long_param->x;
    g_l_new_z.y += multiply(longtmp,    lsiny,   bitshift) + g_long_param->y;
    return longbailout();
#else
    return 0;
#endif
...
MarksLambdaFractal()
{
    ...
    g_l_new_z.x = multiply(g_l_coefficient.x, g_l_temp.x, bitshift)
             - multiply(g_l_coefficient.y, g_l_temp.y, bitshift) + g_long_param->x;
    g_l_new_z.y = multiply(g_l_coefficient.x, g_l_temp.y, bitshift)
...
MarksLambdafpFractal()
{
    ...
    g_new_z.x = g_marks_coefficient.x * tmp.x - g_marks_coefficient.y * tmp.y + g_float_param->x;
    g_new_z.y = g_marks_coefficient.x * tmp.y + g_marks_coefficient.y * tmp.x + g_float_param->y;
...
{
#if !defined(XFRACT)
    XXOne = multiply(g_l_old_z.x, g_l_old_z.x, bitshift) + multiply(g_l_old_z.y, g_l_old_z.y, bitshift);
    if ((XXOne > g_fudge_two) || (labs(XXOne - g_fudge_one) < delmin))
    {
        return 1;
    }
    g_l_old_z.y = multiply(g_fudge_two - XXOne, g_l_old_z.x, bitshift);
    g_l_old_z.x = multiply(g_fudge_two - XXOne, g_l_old_z.y, bitshift);
    g_l_new_z = g_l_old_z;
    return 0;
#else
    return 0;
...
Mandel4fpFractal()
{
    ...
    // then, compute ((x + iy)**2)**2 + lambda
    g_new_z.x  = tempsqrx - tempsqry + g_float_param->x;
    g_new_z.y =  old.x*old.y*2 + g_float_param->y;
    return floatbailout();
}
...
{
    cpower(&old, (int)param[2], &g_new_z);
    old = ComplexPower(old, old);
    g_new_z.x = g_new_z.x + old.x +g_float_param->x;
    g_new_z.y = g_new_z.y + old.y +g_float_param->y;
    return floatbailout();
}
...
{
#if !defined(XFRACT)
    if (lcpower(&g_l_old_z, g_c_exponent, &g_l_new_z, bitshift))
    {
        g_l_new_z.y = 8L << bitshift;
...
floatZpowerFractal()
{
    cpower(&old, g_c_exponent, &g_new_z);
...
{
    g_new_z = ComplexPower(old, parm2);
    g_new_z.x += g_float_param->x;
    g_new_z.y += g_float_param->y;
    return floatbailout();
}
...
Barnsley3Fractal()
{
    ...
    // calculate intermediate products
#if !defined(XFRACT)
    oldxinitx   = multiply(g_l_old_z.x, g_l_old_z.x, bitshift);
    oldyinity   = multiply(g_l_old_z.y, g_l_old_z.y, bitshift);
    oldxinity   = multiply(g_l_old_z.x, g_l_old_z.y, bitshift);
...
Barnsley3FPFractal()
{
    ...
    {
        g_new_z.x = foldxinitx - foldyinity -1.0 + g_float_param->x * old.x;
        g_new_z.y = foldxinity * 2;

        /* This term added by Tim Wegner to make dependent on the
           imaginary part of the parameter. (Otherwise Mandelbrot
           is uninteresting. */
        g_new_z.y += g_float_param->y * old.x;
    }
    return floatbailout();
}
...
PopcornFractal_Old()
{
    ...
    FPUsincos(&tmp.x, &sinx, &cosx);
    FPUsincos(&tmp.y, &siny, &cosy);
    g_new_z.x = old.x - parm.x*siny;
    g_new_z.y = old.y - parm.x*sinx;
    if (plot == noplot)
    {
...
LPopcornFractal_Old()
{
    ...
    g_l_temp.x = divide(lsinx, lcosx, bitshift) + g_l_old_z.x;
    g_l_temp.y = divide(lsiny, lcosy, bitshift) + g_l_old_z.y;
    LTRIGARG(g_l_temp.x);
    LTRIGARG(g_l_temp.y);
    SinCos086(g_l_temp.x, &lsinx, &lcosx);
    SinCos086(g_l_temp.y, &lsiny, &lcosy);
...
PopcornFractalFn()
{
    ...
    g_new_z.x = old.x - tmpx.x - tmpy.y;
    g_new_z.y = old.y - tmpy.x - tmpx.y;
...
int MarksCplxMand()
{
    tmp.x = tempsqrx - tempsqry;
    tmp.y = 2*old.x*old.y;
    FPUcplxmul(&tmp, &g_marks_coefficient, &g_new_z);
    g_new_z.x += g_float_param->x;
    g_new_z.y += g_float_param->y;
    return floatbailout();
}
...
int SpiderfpFractal()
{
    ...
    g_new_z.x = tempsqrx - tempsqry + tmp.x;
    g_new_z.y = 2 * old.x * old.y + tmp.y;
    tmp.x = tmp.x/2 + g_new_z.x;
    tmp.y = tmp.y/2 + g_new_z.y;
    return floatbailout();
}
...
TetratefpFractal()
{
    ...
    g_new_z = ComplexPower(*g_float_param, old);
    return floatbailout();
}
...
ScottZXTrigPlusZfpFractal()
{
    // z = (z*trig(z))+z
    CMPLXtrig0(old, tmp);         // tmp  = trig(old)
    CMPLXmult(old, tmp, g_new_z);       // new  = old*trig(old)
    CMPLXadd(g_new_z, old, g_new_z);        // new  = trig(old) + old
    return floatbailout();
}
...
SkinnerZXTrigSubZfpFractal()
{
    ...
    CMPLXtrig0(old, tmp);         // tmp  = trig(old)
    CMPLXmult(old, tmp, g_new_z);       // new  = old*trig(old)
    CMPLXsub(g_new_z, old, g_new_z);        // new  = trig(old) - old
    return floatbailout();
}
...
LambdaTrigOrTrigfpFractal()
{
    ...
    {
        CMPLXtrig0(old, old);
        FPUcplxmul(g_float_param, &old, &g_new_z);
    }
    else
    {
        CMPLXtrig1(old, old);
...
JuliaTrigOrTrigfpFractal()
{
    ...
    {
        CMPLXtrig0(old, old);
        CMPLXadd(*g_float_param, old, g_new_z);
    }
    else
    {
        CMPLXtrig1(old, old);
...
HalleyFractal()
{
    ...
    g_new_z.x = old.x - Halnumer2.x;
    g_new_z.y = old.y - Halnumer2.y;
...
LongPhoenixFractal()
{
    ...
    g_l_temp.x = multiply(g_l_old_z.x, g_l_old_z.y, bitshift);
    g_l_new_z.x = g_l_temp_sqr_x-g_l_temp_sqr_y+g_long_param->x+multiply(g_long_param->y, ltmp2.x, bitshift);
    g_l_new_z.y = (g_l_temp.x + g_l_temp.x) + multiply(g_long_param->y, ltmp2.y, bitshift);
...
PhoenixFractal()
{
    ...
    g_new_z.y = (tmp.x + tmp.x) + (g_float_param->y * tmp2.y);
...
LongPhoenixFractalcplx()
{
    ...
    // z(n+1) = z(n)^2 + p + qy(n),  y(n+1) = z(n)
    g_l_temp.x = multiply(g_l_old_z.x, g_l_old_z.y, bitshift);
    g_l_new_z.x = g_l_temp_sqr_x-g_l_temp_sqr_y+g_long_param->x+multiply(g_l_param2.x, ltmp2.x, bitshift)-multiply(g_l_param2.y, ltmp2.y, bitshift);
    g_l_new_z.y = (g_l_temp.x + g_l_temp.x)+g_long_param->y+multiply(g_l_param2.x, ltmp2.y, bitshift)+multiply(g_l_param2.y, ltmp2.x, bitshift);
    ltmp2 = g_l_old_z; // set ltmp2 to Y value
    return longbailout();
#else
    return 0;
...
LongPhoenixPlusFractal()
{
    ...
    g_l_new_z.x = lnewminus.x + multiply(g_long_param->y, ltmp2.x, bitshift);
    g_l_new_z.y = lnewminus.y + multiply(g_long_param->y, ltmp2.y, bitshift);
    ltmp2 = g_l_old_z; // set ltmp2 to Y value
    return longbailout();
#else
    return 0;
...
ScottTrigPlusSqrfpFractal() // float version
{
    ...
    CMPLXtrig0(old, g_new_z);       // new = trig(old)
    CMPLXsqr_old(tmp);          // tmp = sqr(old)
    CMPLXadd(g_new_z, tmp, g_new_z);      // new = trig(old)+sqr(old)
    return floatbailout();
}
...
SkinnerTrigSubSqrfpFractal()
{
    ...
    CMPLXsub(g_new_z, tmp, g_new_z);      // new = trig(old)-sqr(old)
    return floatbailout();
}
...
LambdaTrigfpFractal()
{
    FLOATXYTRIGBAILOUT();
    CMPLXtrig0(old, tmp);              // tmp = trig(old)
    CMPLXmult(*g_float_param, tmp, g_new_z);   // new = longparm*trig(old)
    old = g_new_z;
    return 0;
}
...
ManOWarfpFractal()
{
    // From Art Matrix via Lee Skinner
    // note that fast >= 287 equiv in fracsuba.asm must be kept in step
    g_new_z.x = tempsqrx - tempsqry + tmp.x + g_float_param->x;
    g_new_z.y = 2.0 * old.x * old.y + tmp.y + g_float_param->y;
    tmp = old;
    return floatbailout();
}
...
{
    CMPLXtrig0(old, g_new_z);
    g_new_z.x = g_new_z.x * tmp.x - g_new_z.y * tmp.y;
    g_new_z.y = g_new_z.x * tmp.y - g_new_z.y * tmp.x;
    g_new_z.x += g_float_param->x;
    g_new_z.y += g_float_param->y;
    return floatbailout();
}
...
#if !defined(XFRACT)
    LCMPLXtrig0(g_l_old_z, g_l_new_z);
    g_l_new_z.x = multiply(g_l_new_z.x, g_l_temp.x, bitshift)-multiply(g_l_new_z.y, g_l_temp.y, bitshift);
    g_l_new_z.y = multiply(g_l_new_z.x, g_l_temp.y, bitshift)-multiply(g_l_new_z.y, g_l_temp.x, bitshift);
    g_l_new_z.x += g_long_param->x;
    g_l_new_z.y += g_long_param->y;
    return longbailout();
#else
...
int long_julia_per_pixel()
{
    ...
    if (g_invert != 0)
    {
        ...
        g_l_old_z.x = (long)(old.x*g_fudge_factor);
        g_l_old_z.y = (long)(old.y*g_fudge_factor);
    }
    else
    {
...
int marksmandel_per_pixel()
{
    ...
    {
        g_l_coefficient.x = multiply(g_l_old_z.x, g_l_old_z.x, bitshift)
                         - multiply(g_l_old_z.y, g_l_old_z.y, bitshift);
...
VLfpFractal() // Beauty of Fractals pp. 125 - 127
{
    ...
    b = old.y + param[1] * w;
    ab = a * b;
    g_new_z.x = old.x + half * (u + (a - ab));
    g_new_z.y = old.y + half * (w + (-b + ab));
    return floatbailout();
}
