...
void write_batch_parms(char const *colorinf, bool colorsonly, int maxcolor, int ii, int jj)
{
    ...
    if (g_display_3d <= display_3d_modes::NONE)
    {
        ...
        if (fractype == fractal_type::JULIBROT || fractype == fractal_type::JULIBROTFP)
        {
            ...
            // these rarely change
            if (g_julibrot_origin_fp != 8 || g_julibrot_height_fp != 7 || widthfp != 10 || g_julibrot_dist_fp != 24
                    || g_julibrot_depth_fp != 8 || zdots != 128)
            {
                put_parm(" %s=%d/%g/%g/%g/%g/%g", "julibrot3d",
                         zdots, g_julibrot_origin_fp, g_julibrot_depth_fp, g_julibrot_height_fp, widthfp, g_julibrot_dist_fp);
            }
            if (g_eyes_fp != 0)
            {
        ...
        if (g_distance_estimator)
        {
            put_parm(" %s=%ld/%d/%d/%d", "distest", g_distance_estimator, g_distance_estimator_width_factor,
                     g_distance_estimator_x_dots?g_distance_estimator_x_dots:xdots, g_distance_estimator_y_dots?g_distance_estimator_y_dots:ydots);
        }
    ...
    if (g_display_3d != display_3d_modes::NONE)
    {
        ...
        if (g_glasses_type)
        {
            ...
            put_parm(" %s=%d/%d/%d/%d", "crop",
                     red_crop_left, red_crop_right, g_blue_crop_left, g_blue_crop_right);
