...
public:
	FakeDriver() : AbstractDriver(),
		_waitKeyPressedCalled(false), _waitKeyPressedLastTimeout(0), _waitKeyPressedFakeResult(0),
		_fakeMouseMode(0),
		_setMouseModeCalled(false), _setMouseModeLastMode(0),
		_keyStrokes(),
		_keyStrokeCount(0),
		_putStringArgs(),
		_diskpCalled(false), _diskpFakeResult(0),
		_scheduleAlarmCalled(false), _scheduleAlarmLastSeconds(0),
		_resizeCalled(false), _resizeFakeResult(0)
	{
		int argc = 0;
		char *argv[] = { 0 };
	}
	...
	virtual int resize()							{ _resizeCalled = true; return _resizeFakeResult; }
	bool ResizeCalled() const						{ return _resizeCalled; }
	void SetResizeFakeResult(int value)				{ _resizeFakeResult = value; }
	...
	{
		_diskpCalled = true;
		return _diskpFakeResult;
	}
	bool diskpCalled() const { return _diskpCalled; }
	void diskpSetFakeResult(int value) { _diskpFakeResult = value; }
	...
	{
		_scheduleAlarmCalled = true;
		_scheduleAlarmLastSeconds = secs;
	}
	bool ScheduleAlarmCalled() const { return _scheduleAlarmCalled; }
	int ScheduleAlarmLastSeconds() const { return _scheduleAlarmLastSeconds; }
...
private:
	...
	bool _diskpCalled;
	int _diskpFakeResult;
	bool _scheduleAlarmCalled;
	int _scheduleAlarmLastSeconds;
	bool _resizeCalled;
