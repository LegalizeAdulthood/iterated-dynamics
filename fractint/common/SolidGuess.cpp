...
static int guess_row(bool first_pass, int y, int blocksize)
{
	...
	prev11 = -1;
	c22 = get_color(g_ix_start, y);
	c24 = c22;
	c12 = c22;
	c13 = c22;
	c21 = get_color(g_ix_start, (y > 0) ? ylesshalf : 0);
	c31 = c21;
	if (yplusblock <= g_y_stop)
	{
		c24 = get_color(g_ix_start, yplusblock);
	}
	...
	for (int x = g_ix_start; x <= g_x_stop; )  /* increment at end, or when doing continue */
	{
		...
		bool fix21 = ((c22 != c12 || c22 != c32)
			...
			&& y > 0
			&& (x == g_ix_start || c21 == get_color(x-s_half_block, ylessblock))
			&& (xplushalf > g_x_stop || c21 == get_color(xplushalf, ylessblock))
