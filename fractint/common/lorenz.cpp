...
int Linverse_julia_orbit()
{
	...
	//
	newcol = int((multiply(s_lcvt.a, g_new_z_l.real() >> (g_bit_shift - 21), 21) +
			multiply(s_lcvt.b, g_new_z_l.imag() >> (g_bit_shift - 21), 21) + s_lcvt.e) >> 21);
	newrow = int((multiply(s_lcvt.c, g_new_z_l.real() >> (g_bit_shift - 21), 21) +
			multiply(s_lcvt.d, g_new_z_l.imag() >> (g_bit_shift - 21), 21) + s_lcvt.f) >> 21);
	...
	if (newcol < 1 || newcol >= g_x_dots || newrow < 1 || newrow >= g_y_dots)
	{
		...
		case MAJORMETHOD_BREADTH_FIRST:
			g_new_z_l = ComplexSqrtLong(g_new_z_l.real() - s_x_long, g_new_z_l.imag() - s_y_long);
			EnQueueLong(color*g_new_z_l.real(), color*g_new_z_l.imag());
			break;
		case MAJORMETHOD_DEPTH_FIRST:
			g_new_z_l = ComplexSqrtLong(g_new_z_l.real() - s_x_long, g_new_z_l.imag() - s_y_long);
			PushLong(color*g_new_z_l.real(), color*g_new_z_l.imag());
			break;
		case MAJORMETHOD_RANDOM_RUN:
			random_len--;
		case MAJORMETHOD_RANDOM_WALK:
	...
	case MAJORMETHOD_BREADTH_FIRST:
		if (color < s_max_hits)
		{
			g_plot_color_put_color(newcol, newrow, color + 1);
			g_new_z_l = ComplexSqrtLong(g_new_z_l.real() - s_x_long, g_new_z_l.imag() - s_y_long);
			EnQueueLong(g_new_z_l.real(),  g_new_z_l.imag());
			EnQueueLong(-g_new_z_l.real(), -g_new_z_l.imag());
		}
		break;
	case MAJORMETHOD_DEPTH_FIRST:
		if (color < s_max_hits)
		{
			...
			if (g_minor_method == MINORMETHOD_LEFT_FIRST)
			{
				if (QueueFullAlmost())
				{
					PushLong(-g_new_z_l.real(), -g_new_z_l.imag());
				}
				else
				{
					PushLong(g_new_z_l.real(),  g_new_z_l.imag());
