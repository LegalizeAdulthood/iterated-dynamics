...
TEST(Axiom, LSystemParser)
{
	...
	LSystemParser parser = LSystemParser::StackInstance();
	CHECK(parser.Parse(axiom));
	LONGS_EQUAL(1, parser.Count());
	const LSystemEntry *entry = parser.Entry(0);
	CHECK_EQUAL("Axiom", entry->Id());
...
TEST(Production, LSystemParser)
{
	...
	LSystemParser parser = LSystemParser::StackInstance();
	CHECK(parser.Parse(production));
	LONGS_EQUAL(1, parser.Count());
	const LSystemEntry *entry = parser.Entry(0);
	CHECK_EQUAL("Production", entry->Id());
...
TEST(Production2, LSystemParser)
{
	const std::string production2 = "Production2 {\n"
		...
		"}\n";
	LSystemParser parser = LSystemParser::StackInstance();
	CHECK(parser.Parse(production2));
	LONGS_EQUAL(1, parser.Count());
	const LSystemEntry *entry = parser.Entry(0);
	CHECK_EQUAL("Production2", entry->Id());
	LONGS_EQUAL(3, entry->ProductionCount());
	{
		const LSystemProduction rule = entry->Production(0);
		CHECK_EQUAL("A", rule.Symbol());
		CHECK_EQUAL("BC", rule.Production());
	}
	{
		const LSystemProduction rule = entry->Production(1);
		CHECK_EQUAL("B", rule.Symbol());
		CHECK_EQUAL("F-F", rule.Production());
	}
	{
		const LSystemProduction rule = entry->Production(2);
		CHECK_EQUAL("C", rule.Symbol());
		CHECK_EQUAL("F+F", rule.Production());
	}
}
...
TEST(Koch1, LSystemParser)
{
	...
	CHECK(parser.Parse(koch1));
	LONGS_EQUAL(1, parser.Count());
	const LSystemEntry *entry = parser.Entry(0);
	CHECK_EQUAL("Koch1", entry->Id());
...
TEST(Snowflake2, LSystemParser)
{
	...
	CHECK(parser.Parse(snowflake2));
	LONGS_EQUAL(2, parser.Count());
	const LSystemEntry *entry0 = parser.Entry(0);
	CHECK_EQUAL("Snowflake2", entry0->Id());
