...
calc_mand_assembly(void)
{
    __asm
    {
        ...
        mov     edx, eax                    ;
        clear edx
        cmp     periodicitycheck, eax       ;
        periodicity checking disabled?
        je      initoldcolor                ;
        yup, set oldcolor 0 to disable it
        cmp     reset_periodicity, eax      ;
        periodicity reset?
        je      short initparms             ;
        inherit oldcolor from prior invocation
        mov     eax, dword ptr maxit        ;
        yup.  reset oldcolor to maxit-250
        sub     eax, 250                    ;
        (avoids slowness at high maxits)

        initoldcolor:
        mov     dword ptr oldcoloriter, eax ;
        reset oldcoloriter

        initparms:
        mov     eax, dword ptr c_real       ;
        initialize    x == c_real
        mov     dword ptr x, eax            ;
        ...

        mov     eax, dword ptr c_imag       ;
        initialize    y == c_imag
        mov     dword ptr y, eax            ;
        ...
        ...
        sub     dword ptr k, 1              ;
        we know the first iteration passed
        sbb     dword ptr k+4, 0            ;
        we know the first iteration passed
        mov     edx, dword ptr linitx+4     ;
        add x += linitx
        ...
        ...
        jmp     short doeither              ;
        branch around the julia switch

        dojulia:
        ;
    Julia Set initialization
    ; "fudge"
    Mandelbrot start-up values
    mov     eax, dword ptr x            ;
    switch    x with linitx
    ...
    doeither:
    ;
    common Mandelbrot, Julia set code
    mov     period, 0                   ;
    claim periodicity of  1
    mov     eax, dword ptr firstsavedand ;
    initial periodicity check
    mov     dword ptr savedand, eax     ;
    initial periodicity check
    mov     eax, dword ptr firstsavedand+4 ;
    initial periodicity check
    mov     dword ptr savedand+4, eax   ;
    initial periodicity check
    mov     savedincr, 1                ;
    flag for incrementing periodicity
        mov     dword ptr savedx+4, 0ffffh  ;
    impossible value of "old" x
    mov     dword ptr savedy+4, 0ffffh  ;
    impossible value of "old" y
    mov     orbit_ptr, 0                ;
    clear orbits

    dec     kbdcount                    ;
    decrement the keyboard counter
    jns     nokey                       ;
    skip keyboard test if still positive
        mov     kbdcount, 10                ;
    stuff in  a low kbd count
    cmp     show_orbit, 0               ;
    are we showing orbits?
    jne     quickkbd                    ;
    yup.  leave it that way.
    cmp     orbit_delay, 0              ;
    are we delaying orbits?
    je      slowkbd                     ;
    nope.  change it.
    cmp     showdot, 0                  ;
    are we showing the current pixel?
    jge     quickkbd                    ;
    yup.  leave it that way.
    slowkbd:
    mov     kbdcount, 5000              ;
    else, stuff an appropriate count  val
    cmp     cpu, 386                    ;
    ("appropriate" to the CPU)
        jae     short kbddiskadj            ;
        ...
        cmp     dword ptr delmin+4, 8       ;
        is 16-bit math good enough?
        ja      kbddiskadj                  ;
        yes. test less often
        mov     kbdcount, 500               ;
        no.  test more often
        ...
        nope.  proceed
        mov     kbdcount, 0                 ;
        make  sure it goes negative again
        cmp     eax, 'o'                    ;
        orbit toggle hit?
        je      orbitkey                    ;
        yup.  show orbits
        ...
        orbitkey:
        call     driver_get_key             ;
        read the key for real
        mov     eax, 1                      ;
    reset orbittoggle = 1 - orbittoggle
    sub     eax, show_orbit             ;
    ...
    mov     show_orbit, eax             ;
    ...
    jmp     short nokey                 ;
    pretend no key was hit
    keyhit:
    mov     eax, -1                     ;
    return with -1
    mov     edx, eax
    mov     dword ptr coloriter, eax    ;
    set coloriter to -1
    ...
    cmp     cpu, 386                    ;
    are we on a 386?
    jae     short code386bit            ;
    YAY!! 386-class speed!
        cmp     dword ptr delmin+4, 8       ;
    OK, we're desperate.  16 bits OK?
    ...
    while (k < maxit)
        ...
        chkpd386_16:
        test    eax, savedand               ;
        save  on 0, check on anything else
            jz      short chksv386_16           ;
        time to save a new "old" value
        mov     ebx, esi                    ;
        load up x
        xor     ebx, dword ptr savedx+4     ;
        does X    match?
        cmp     ebx, dword ptr lclosenuff+4 ;
        truncate to appropriate precision
        ja      short nonmax386_16          ;
        nope.  forget it.
        mov     ebx, edi                    ;
        now test y
        xor     ebx, dword ptr savedy+4     ;
        does Y    match?
        cmp     ebx, dword ptr lclosenuff+4 ;
        truncate to appropriate precision
        ja      short nonmax386_16          ;
        nope.  forget it.
        mov     period, 1                   ;
        note  that we have found periodicity
        mov     k, 0                        ;
        pretend maxit reached
        jmp     short end386_16
        chksv386_16:
        mov     dword ptr savedx+4, esi     ;
        save x
        mov     dword ptr savedy+4, edi     ;
        save y
        dec     savedincr                   ;
        time to change the periodicity?
        jnz     short nonmax386_16          ;
        nope.
        shl     savedand, 1                 ;
        ...
        dec     k                           ;
        while (k < maxit)(dec to 0 is faster)
            jnz     kloop                       ;
        while (k < maxit) ...
            kloopend1:
            jmp     short kloopend32            ;
        we done.
    ...
    ;       detected by the dull-normal maximum iteration process).
        ...
        periodicity check
        mov     eax, dword ptr k            ;
        set up to test for save-time
        test    eax, dword ptr savedand     ;
    save on 0,    check on anything else
        jnz     notimeyet                   ;
    NOT time to save a new "old" value
    mov     edx, dword ptr k+4 ;
    set up to test for save-time
        test    edx, dword ptr savedand+4   ;
    save on 0,    check on anything else
        jz      checksave                   ;
    time to save a new "old" value
    notimeyet:
    ...
    mov     edx, dword ptr x+4          ;
    load up x
    mov     dword ptr savedx+4, edx     ;
    and save it
    mov     eax, dword ptr x            ;
    load up x
    mov     dword ptr savedx, eax       ;
    and save it
    mov     edx, dword ptr y+4          ;
    load up y
    mov     dword ptr savedy+4, edx     ;
    and save it
    mov     eax, dword ptr y            ;
    load up y
    mov     dword ptr savedy, eax       ;
    and save it
    dec     savedincr                   ;
    time to change the periodicity?
    jnz     checkdone                   ;
    nope.
    shl     dword ptr savedand, 1       ;
    well then, let's try this one!
    rcl     dword ptr savedand+4, 1     ;   well then, let's try this one!
        add     dword ptr savedand, 1       ;
    (2**n +1)
        adc     dword ptr savedand+4, 0     ;
        (2**n +1)
        mov     eax, nextsavedincr          ;
        and reset the increment flag
        mov     savedincr, eax              ;
        and reset the increment flag
        checkdone:
        ret                                 ;
        ...
        ******************** Function code32bit() *****************************
        ...
        ;
        As we prescaled DI:
        AX, we need to shift the answer 1 bit to the right to
        ;
    ...
    eax=low(y)
        ...
        ;
        square done1                       ;
        square y and quit via done1 if it overflows
        shl     eax, 1                      ;
    Multiply   by 2 to convert to fg30
    rcl     edi, 1                      ;
    If this overflows DI:
    AX was negative
    jnc     notneg
    not     eax                         ;
    so negate it
    ...
    notneg:
    shl     eax, 1                      ;
    Multiply   by 2 again to give fg31
    rcl     edi, 1                      ;
    If this gives a carry then DI:
    AX   was >=2.0
    ;
    If its high bit is set then DI:
    AX was >=1.0
    ;
    This is OK, but note that this means that
    ;
    DI:
    AX must now be treated as unsigned.
    jc      done1
    push    edi                         ;
    save  y or x(in fg31 form) on stack
        push    eax                         ;
        ...
        mul     edi                         ;
        GET MIDDLE PART -  2*A*B
        mov     bh, ah                      ;
        Miraculously,  it needs no shifting!
        mov     ecx, edx
        mov     eax, edi
        mul     eax                         ;
        SQUARE HIGH HWORD  - A*A
        shl     bh, 1                       ;
        See if we  round up
        adc     eax, 1                      ;
    Anyway, add 1 to   round up/down accurately
    adc     edx, 0
    shr     edx, 1                      ;
    This   needs shifting one bit
    rcr     eax, 1
    add     eax, ecx                    ;
    Add    in the 2*A*B term
    adc     edx, 0

    mov     esi, eax                    ;
    square    returns results in edx, eax
    mov     ebp, edx                    ;
    save y*y in ebp, esi
    mov     eax, dword ptr x
    ...
    ;
    square  done2                      ;
    square x  and quit via done2 if it overflows
        shl     eax, 1                      ;
    Multiply   by 2 to convert to fg30
    rcl     edi, 1                      ;
    If this overflows DI:
    ...
    ;
    NOTE:
    The original code tests against lm
    ;
    ...
    as testing for signed overflow
        done4:
        add     sp, 4                       ;
    discard saved value of |x| fg 31
    done2:
    add     sp, 4                       ;
    discard saved value of |y| fg 31
    done1:
    xor     ecx, ecx                    ;
    no    key exit, zero ecx
    done0:
    ;
    exit here if key hit
        pop     ebp                         ;
    restore saved ebp
    ret
    ...
    while (k < maxit)
        ...
        tryagain:
        sub     ecx, esi                    ;
        subtract y*y from x*x
        sbb     edx, ebp                    ;
        ...
        add     ecx, dword ptr linitx       ;
        add "A"
        adc     edx, dword ptr linitx+4     ;
        ...
        jo      done4                       ;
        CJLT-Must detect overflow here
        ;
        but increment loop count first
        mov     dword ptr x, ecx            ;
        store new x = x*x-y*y+a
        ...
        Xlow
        pop     edi                         ;
        Xhigh(already there, actually)
        pop     eax                         ;
        Ylow
        mul     edi                         ;
        Xhigh  * Ylow
        mov     bh, ah                      ;
        Discard lowest 8 bits  of answer
        mov     ecx, edx
        pop     eax                         ;
        Yhigh
        mov     esi, eax                    ;
        ...
        ;
        NOTE:
        The answer is 0..3.9999 in fg29
        js      done1                       ;
        Overflow if high bit set
        or      bl, bl                      ;
    ZERO  IF NONE OR BOTH X , Y NEG
    jz      signok                      ;
    ONE IF ONLY ONE OF X OR Y IS NEG
    not     eax                         ;
    negate result
    not     edx                         ;
    ...
    ...
    adc     edx, dword ptr linity+4     ;
    edx, eax =    2(X*Y)+B
        jo      done1
        mov     dword ptr y, eax            ;
        save the new value    of y
        mov     dword ptr y+4, edx          ;
        ...
        mov     edx, dword ptr oldcoloriter+4 ;
        recall  the old color
        cmp     edx, dword ptr k+4          ;
        check it against this iter
        jb      short chkkey4               ;
        nope.  bypass periodicity check.
        mov     eax, dword ptr oldcoloriter ;
        recall    the old color
        cmp     eax, dword ptr k            ;
        check it against this iter
        jb      short chkkey4               ;
