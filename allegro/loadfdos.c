...
{
   static FCODE o_hdg2[]={"key.driver....................xdot..ydot..clr.rgba.comment.................."};
...
Press F1 for help, "};
   ...
   for (i = 0; i < vidtbllen; ++i) {
      ...
      if (info->xdots == vident->xdots && info->ydots == vident->ydots
          ...
          && info->videomodeR == vident->videomodeR
          && info->videomodeG == vident->videomodeG
          && info->videomodeB == vident->videomodeB
          && info->videomodeA == vident->videomodeA
   ...
   if (gotrealmode == 0) { /* translate from temp table to permanent */
      ...
      if (j == 0) /* mode has no key, add to reserved slot at end */
         far_memcpy((char far *)&videotable[initmode=MAXVIDEOTABLE-1],
                    (char far *)&vidtbl[i],sizeof(videotable[0]));
      }
