...
/* ##FIXME## */
   ...
   {                            /* store non-standard fractal info */
      ...
      if (resume_info != 0 && save_info.calc_status == 2)
      {
         ...
         MoveFromMemory((BYTE *)block,(U16)1,(U16)resume_len,0,resume_info);
...
int _fastcall new_to_old(int new_fractype)
{
   /* converts from new to old fractal types */
   static SIFCODE new2old[] = 
   {
        4,  67,  66,   6,   7,   8,   9,  11,  18,  26,  27, 
       31,  32,  33,  35,  37,  39,  40,  42,  47,  48,  49, 
       53,  54,  55,  56,  59,  60,  61,  63,  68,  69,  70, 
       71,  73,  74,  75,  76,  77,  78,  79,  80,  81,  82, 
       84,  86,  88,  89,  90,  91,  92,  93,  95,  96,  97, 
       98,  99, 102, 103, 104, 105, 107, 108, 110, 112, 114, 
      118, 120, 121, 122, 123, 124, 125, 126, 128, 130, 132, 
      134, 136, 138, 139, 140, 141, 142, 143, 145, 146, 148, 
      150, 151, 152, 154, 155, 156, 157, 158, 159, 160, 162, 
      164, 165, 166, 167, 168, 169, 170, 171 
   };

   return new2old[new_fractype];
}
...
static void _fastcall setup_save_info(struct fractal_info far * save_info)
{
   ...
   save_info->fractal_type = (short) new_to_old(fractype);
   ...
   save_info->videomodeR = (short) videoentry.videomodeR;
   save_info->videomodeG = (short) videoentry.videomodeG;
   save_info->videomodeB = (short) videoentry.videomodeB;
   save_info->videomodeA = (short) videoentry.videomodeA;
