...
/* #define PUT_KEY(name, descrip) driver_put_string(-1,-1,C_HELP_INSTR_KEYS,name), driver_put_string(-1,-1,C_HELP_INSTR," "descrip"  ") */
/* ##FIXME## */
#ifndef XFRACT
...
static void helpinstr(void)
   ...
   for (ctr=0; ctr<SCREEN_WIDTH; ctr++)
...
static void display_page(char far *title, char far *text, unsigned text_len, int page, int num_pages, int start_margin, int *num_link, LINK far *link)
   ...
   driver_put_string(1, SCREEN_WIDTH-(6 + ((num_pages>=10)?2:1)), C_HELP_INSTR, temp);
