...
/* Added the following to accommodate fn bifurcations.  JCO 7/2/92 */
   if(fractype == BIFURCATION && oldfractype != BIFURCATION) 
        set_trig_array(0,s_ident);
   if(fractype == BIFSTEWART && oldfractype != BIFSTEWART)
        set_trig_array(0,s_ident);
   if(fractype == BIFLAMBDA && oldfractype != BIFLAMBDA)
        set_trig_array(0,s_ident);
   if(fractype == BIFEQSINPI && oldfractype != BIFEQSINPI)
        set_trig_array(0,s_sin);
   if(fractype == BIFADSINPI && oldfractype != BIFADSINPI)
        set_trig_array(0,s_sin);
 ...
 */
   ...
   typename = curfractalspecific->name;
...
static int check_gfe_key(int curkey,int choice)
{
/* FIXME this routine still does not display correctly JCO */
   char infhdg[60];
   char far *infbuf;
 ...
 /* ... instead, call help with buffer?  heading added */
      ...
      {
      static FCODE msg[]  = {"\n\n Use "UPARR1", "DNARR1", "RTARR1", "LTARR1", PgUp, PgDown, Home, and End to scroll\nAny other key to return to selection list"};
