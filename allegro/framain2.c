...
static void julman()
{
   ...
   {
      if(fractalspecific[i].tojulia != NOFRACTAL)
         fprintf(fp,"%s  %s\n",fractalspecific[i].name,
...
#endif
            if (calc_status == 2 && zwidth == 0 && !fkeypressed()) {
...
/* ##FIXME## */
   ...
   case 'w':
...
#if 0
      {
         FILE *blob;
         int i = -1;
         blob = fopen("blob.txt","w");
         while(fractalspecific[++i].name != NULL)
         {
            fprintf(blob,"/* %3d %-16s */\n",i,fractalspecific[i].name);
         }
         fclose(blob);
      }
#endif
   ...
   case SPACE:                  /* spacebar, toggle mand/julia   */
      ...
      else
      {
             ...
             && param[0] == 0.0 && param[1] == 0.0)
         {
            ...
            int key;
            if (fractype == MANDELFP && bf_math == 0)
                  hasinverse = 1;
            else
                  hasinverse = 0;
            clear_zoombox();
            ...
            *frommandel = 1;
            ...
            xx3rd = xxmin;
            yy3rd = yymin;
            if (usr_distest == 0 && usr_biomorph != -1)
            {
               xxmin *= 3.0;
               xxmax *= 3.0;
               yymin *= 3.0;
...
*/
   ...
   case DELETE:         /* select video mode from list */
   {
      ...
      *kbdchar = select_video_mode(adapter);
      if (*kbdchar >= 0) { /* picked a new mode? */
         driver_discard_screen();
         adapter = *kbdchar;
   ...
   while (getmore) {
      ...
      if (getmore) {
         if (getmore == 2)              /* eat last key used */
            getakey();
         getmore = 2;
         keynum = fkeypressed();         /* next pending key */
         }
      }
   if (boxcount) {
/*
