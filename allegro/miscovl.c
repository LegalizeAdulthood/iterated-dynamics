...
static void write_batch_parms(char *colorinf, int colorsonly, int maxcolor, int ii, int jj)
{
   ...
   if (display3d <= 0) { /* a fractal was generated */
      ...
      put_parm( s_seqs,s_type,curfractalspecific->name);
...
/*          convert 1000 fudged long to double, 1000/1<<24 = 6e-5 */
      ...
      for(i = (MAXPARAMS-1); i >= 0; --i)
          if(typehasparm(fractype==JULIBROTFP?neworbittype:fractype,i,NULL)) 
             break;
      if (i >= 0) {
...
{
   static FCODE o_hdg2[]={"device...........xdot..ydot.colors.rgba..comment.................."};
...
#if 1
   load_fractint_cfg(0);        /* load fractint.cfg */
#endif
   ...
   for (i = 0; i < vidtbllen; ++i) { /* find default mode */
      if (videoentry.colors      == vidtbl[entnums[i]].colors
          && (curmode < 0
   ...
   i = fullscreen_choice(CHOICEHELP,hdg1,hdg2,NULL,vidtbllen,NULL,attributes,
                         1,16,74,i,format_vid_table,NULL,NULL,NULL);
   tabmode = oldtabmode;
   helpmode = oldhelpmode;
#if 0
   if (i == -1) {
...
#endif
   if (i >= 0)
      i = entnums[i]; /* picked by Enter key */
   else
      return(-1);
...
void format_vid_table(int choice,char *buf)
{
   ...
   far_memcpy((char far *)&videoentry,(char far *)&vidtbl[entsptr[choice]],
              sizeof(videoentry));
   sprintf(buf,"%-15s %5d %5d ",  /* 28 chars */
           videoentry.name, videoentry.xdots, videoentry.ydots);
   if((truecolorbits = videoentry.truecolorbits) == 8)
      sprintf(local_buf,"%s %3d",  /* 32 chars */
           buf, videoentry.colors);
   else
      sprintf(local_buf,"%s %3s",  /* 32 chars */
           buf,(truecolorbits == 32)?" 4g":
                (truecolorbits == 24)?"16m":
                (truecolorbits == 16)?"64k":
                (truecolorbits == 15)?"32k":"???");
   sprintf(local_buf,"%s   %1i%1i%1i%1i ",  /* 40 chars */
           local_buf, videoentry.videomodeR, videoentry.videomodeG,
           videoentry.videomodeB, videoentry.videomodeA);
   sprintf(buf,"%s %-25s",  /* 65 chars */
...
{
   int i,j;
   if ((i = vidtbl[*((int *)p1)].xdots) == 0) i = 9999;
   if ((j = vidtbl[*((int *)p2)].xdots) == 0) j = 9999;
...
update_fractint_cfg(void)
{
   ...
   for (k = 0 ; k < vidtbllen; k++) {
      ...
      char colorsbuf[10];
      char buf[121];
      memcpy((char *)&vident,(char *)&vidtbl[k], sizeof(videoentry));
      strcpy(buf,vident.name);
      i = strlen(buf);
      while (i && buf[i-1] == ' ') /* strip trailing spaces to compress */
         --i;
      j = i + 5;
      while (j < 22) {               /* tab to column 23 */
         buf[i++] = '\t';
         j += 8;
         }
      buf[i] = 0;
      if((truecolorbits = vident.truecolorbits) == 8)
         ...
         sprintf(colorsbuf,"%3s",
            ...
            (truecolorbits == 16)?"64k":
            (truecolorbits == 15)?"32k":"???");
      fprintf(cfgfile,"%s,%5u,%5u,%s,%1u,%1u,%1u,%1u,%s\n",
             buf,
             vident.xdots,
             vident.ydots,
             colorsbuf,
