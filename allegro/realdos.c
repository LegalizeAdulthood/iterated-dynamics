...
#ifdef FLOATONLY
static char far s_incremental[] = {"Float-only version"};
#else
...
int stopmsg (int flags, char far *msg)
{
   ...
   while (fkeypressed()) /* flush any keyahead */
      getakey();
   if(debugflag != 324)
...
int texttempmsg(char far *msgparm)
{
   if (showtempmsg(msgparm))
      return(-1);
#ifndef XFRACT
   while (!fkeypressed()) ; /* wait for a keystroke but don't eat it */
#else
...
void helptitle()
{
   ...
   driver_set_clear(); /* clear the screen and set for text */
#ifdef XFRACT
   strcpy(msg,"X");
#else
   *msg=0;
...
#endif
   driver_put_string(0,55,C_TITLE_DEV,s_incremental);
#endif
}
...
void footer_msg(int *i, int options, char *speedstring)
{
   ...
   static FCODE choiceinstr2b[]="Press ENTER for highlighted choice, ESC to back out, or F1 for help";
                   ...
                   int (*speedprompt)(int,int,int,char *,int))
{
   ...
   if (*speedstring) {                 /* got a speedstring on the go */
      ...
      driver_move_cursor(speedrow,17+j+strlen(speedstring));
      }
   else
...
#ifndef XFRACT
      while (!fkeypressed()) { } /* enables help */
#else
...
top:
   ...
   if (fullmenu) {
      ...
      if(!(fractype==JULIAFP || fractype==INVERSEJULIAFP))
          nextleft+=2;
      }
...
/* Don't tie video modes to keys anymore.
