...
static void corners(DMATRIX, int, double *, double *, double *, double *, double *, double *);
static void draw_light_box(double *, double *, DMATRIX);
...
DMATRIX m; /* transformation matrix */
int Ambient;
int RANDOMIZE;
int haze;
   ...
   if (!col && RAY && currow != 0)
      ...
      while (col < (int) linelen)
      {
         ...
         case 4:                /* "solid fill" */
            ...
            {
	      /* This is obviously broken since we are removing the integer math. */
	      /* The fix isn't so obvious. JCO */
               lv[0] = col;
               lv[1] = currow;
               lv[2] = 0;
         ...
         case 6:
            ...
            if (currow && col)  /* skip first row and first column */
            {
               ...
               fcross_product(v1, v2, cross);

               /* normalize cross - and check if non-zero */
               if (fnormalize_vector(cross))
               {
                  if (debugflag)
                  {
...
static void corners(DMATRIX m, int show, double *pxmin, double *pymin, double *pzmin, double *pxmax, double *pymax, double *pzmax)
{
   int i, j;
...
static void draw_light_box(double *origin, double *direct, DMATRIX light_m)
{
   VECTOR S[2][4];
   int i, j;
   double temp;
...
{
   int err;
   DMATRIX lightm;               /* m w/no trans, keeps obj. on screen */
   ...
   fnormalize_vector(light_direction);

   if (preview && showbox)
   {
      fnormalize_vector(direct);
