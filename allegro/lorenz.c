...
struct float3dvtinf /* data used by 3d view transform subroutine */
{
   ...
   double maxvals[3];
   double minvals[3];
   DMATRIX doublemat;    /* transformation matrix */
   DMATRIX doublemat1;   /* transformation matrix */
   int row,col;         /* results */
   int row1,col1;
   struct affine cvt;
};
...
static void setupmatrix(DMATRIX);
...
static int ifs2d(void)
{
   ...
   while(coloriter++ <= maxct) /* loop until keypress or maxit */
   {
      if( fkeypressed() )  /* keypress bails out */
      {
         ret = -1;
         break;
      }
...
static void setupmatrix(DMATRIX doublemat)
{
