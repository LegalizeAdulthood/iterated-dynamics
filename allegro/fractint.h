...
struct videoinfo {              /* All we need to know about a Video Driver */
        char    name[16];       /* Driver name (Allegro, X11, etc)      */
        char    comment[26];    /* Comments (UNTESTED, etc)             */
        int     keynum;         /* key number used to invoked this mode */
                                /* 2-10 = F2-10, 11-40 = S,C,A{F1-F10}  */
        int     videomodeR;     /* begin with R=(this)                  */
        int     videomodeG;     /*            G=(this)                  */
        int     videomodeB;     /*            B=(this)                  */
        int     videomodeA;     /*            A=(this)                  */
                                /* NOTE: IF R==G==B==A==0, uses palette */
        int     truecolorbits;  /* number of truecolor bits             */
...
#else
#define MAXVIDEOTABLE 10        /* size of the resident video modes table */
#endif
...
typedef double DMATRIX [RMAX] [CMAX]; /* matrix of doubles */
...
#undef TRUE
#endif
/* we need TRUE = 1 and FALSE = 0 as indices into an array */
#define TRUE 1
#define FALSE 0
