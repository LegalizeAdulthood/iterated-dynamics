...
x11_resize(Driver *drv)
{
  ...
  if (resize_flag) {
     ...
     resize_flag = 0;
     XQueryTree(di->Xdp, di->Xw, &root, &parent, &children, &junkui);
     if (!parent) return 0;
     XGetGeometry(di->Xdp, parent, &root, &junki, &junki,
         &width, &height, &junkui, &junkui);
     XResizeWindow(di->Xdp, di->Xw, width, height);
     x11_flush(drv);
     usleep(100000);
  } else
  ...
  if (oldx != width || oldy != height) {
    ...
    if (di->Xdepth==1)
       ...
       Xmwidth = sxdots;
    else if (di->Xdepth<=16) {  /* 15 or 16 bpp */
       Xmwidth = 2*sxdots;
       Xpad = 16;
    } else {  /* 24 or 32 bpp */
       Xmwidth = 4*sxdots;
...
x11_write_pixel(Driver *drv, int x, int y, int color)
{
  ...
  DIX11(drv);
    ...
    real_color = FAKE_LUT(di, di->pixtab[color]);
	...
	XSetForeground(di->Xdp, di->Xgc, real_color);
	di->xlastcolor = color;
    }
    XPutPixel(di->Ximage, x, y, real_color);
...
x11_set_line_mode(Driver *drv, int mode)
{
  ...
  if (mode == 0) {
    ...
    XSetForeground(di->Xdp, di->Xgc, WhitePixel(di->Xdp, di->Xdscreen));
    di->xlastcolor = -1;
...
handle_esc(DriverX11 *di)
{
  ...
  ch1 = getachar(di);
  if (ch1 == -1) {
    xdelay(250); /* Wait 1/4 sec to see if a control sequence follows */
    ch1 = getachar(di);
  }
  if (ch1 == -1)
    return ESC;
 ...
 */
static int
ev_key_press(DriverX11 *di, XKeyEvent *xkey)
{
  int charcount;
  char buffer[1];
  KeySym keysym;
...
/* ev_key_release
 *
 * toggle modifier key state for shift and contol keys, otherwise ignore
 */
static void
ev_key_release(DriverX11 *di, XKeyEvent *xkey)
{
  char buffer[1];
  KeySym keysym;
  (void) XLookupString(xkey, buffer, 1, &keysym, NULL);
  switch (keysym) {
    case XK_Control_L:
    case XK_Control_R:
      di->ctl_mode = 0;
...
ev_expose(DriverX11 *di, XExposeEvent *xevent)
{
  if (di->text_modep) {
    ...
    if (x < sxdots && y < sydots && w > 0 && h > 0) {
      XPutImage(di->Xdp, di->Xw, di->Xgc, di->Ximage, x, y, x, y, w, h);
    }
  }
}
...
static void
ev_configure_notify(Driver *drv, XEvent *xevent)
{
  DIX11(drv);
   XSelectInput(di->Xdp,di->Xw,KeyPressMask|KeyReleaseMask|ExposureMask|
...
handle_events(Driver *drv)
{
  ...
  while (XPending(di->Xdp) && !di->xbufkey) {
    ...
    case ConfigureNotify:
      ev_configure_notify(drv, &xevent);
      break;
...
x11_window(Driver *drv)
{
  ...
  di->size_hints = XAllocSizeHints();
  if (di->size_hints == NULL) {
  ...
  if (di->Xgeometry) {
     di->size_hints->flags = USSize | PBaseSize | PResizeInc;
  }
  else {
     di->size_hints->flags = PSize | PBaseSize | PResizeInc;
  }
  di->size_hints->base_width = 320;
  di->size_hints->base_height = 200;
  di->size_hints->width_inc = 4;
  di->size_hints->height_inc = 4;
  ...
  if (di->Xgeometry && !di->onroot) {
    ...
    XWMGeometry(di->Xdp, di->Xdscreen, di->Xgeometry, DEFXY, 0, di->size_hints,
	      &Xwinx, &Xwiny, &di->Xwinwidth, &di->Xwinheight, &di->gravity);
  }
  if (di->sync)
...
/*
; ***************** Function delay(long delaytime) ************************
;
;       performs a delay loop for 'delaytime' milliseconds
*/
static void
x11_delay(Driver *drv, long time)
{
  xdelay((int)time);
}
