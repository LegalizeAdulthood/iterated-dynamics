...
{
   int i;
   if (driver_diskp()) { /* ditch the original disk file, make just the targa */
...
int _fastcall near common_startdisk(long newrowsize, long newcolsize, int colors)
{
   ...
   if (driver_diskp()) { /* otherwise, real screen also in use, don't hit it */
	 ...
	 driver_set_attr(BOXROW+i,BOXCOL,C_DVID_LO,BOXWIDTH);  /* init box */
      ...
      driver_put_string(BOXROW+4,BOXCOL+4,C_DVID_LO,"Screen resolution: ");
	 ...
	 driver_put_string(-1,-1,C_DVID_LO,buf);
      ...
      driver_put_string(BOXROW+8,BOXCOL+4,C_DVID_LO,"Status:");
