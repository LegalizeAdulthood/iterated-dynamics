...
/* ##FIXME## probably for skipping credits in xfractint mode */
#ifdef XFRACT
...
/* ##FIXME## -- should probably do some interaction with the list of
   available drivers to deal with 'adapter=' command */
#ifndef XFRACT
...
/* ##FIXME## -- should use display driver to report the message, not
   printf */
#ifndef WINFRACT
#ifndef XFRACT
         if(*readname != 0)
...
/* keep this next part in for backwards compatibility with old PARs ??? */
   ...
   if (far_strcmp(variable,s_sound) == 0 ) {        /* sound=?,?,? */
      ...
      if (charval[0] == 'n' || charval[0] == 'o')
         ...
         goto badarg;
/* ##OS## -- deal with sound in O/S specific manner rather than ifdef */
#ifndef XFRACT
      if (totparms > 1) {
       int i;
         ...
         for (i = 1; i < totparms; i++) {
	      ...
	      /* these sound flags should have an enum...--RT */
               if (driver_init_fm())
