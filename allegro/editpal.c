...
/* ##FIXME##
   editpal_cursor is used to deal with mouse tracking, apparently an
   event-delivery interaction */
#ifdef XFRACT
...
void
displayc(int x, int y, int fg, int bg, int ch)
{
   int xc, yc;
   char msg[2];
#if 0
...
#else
/* ##FIXME## The passed x and y are pixel dimensions.
   We should be using character dimensions (row,col) instead.
   This will require the change of all the calls to displayc.
   Also, the spacing of box lines will need to be adjusted. JCO */
  msg[0] = (char)ch;
  msg[1] = 0;
  yc = y / 8;
  xc = 1 + (x / 8);
  driver_put_string(yc, xc, fg&15, msg);
#endif
}
   ...
   while ( !done )
      ...
      if (!done)
         ...
         if (!first)
            ...
            first = FALSE;
         key = fkeypressed();   /* peek at the next one... */
         }
      }
...
typedef void other_key_proc_t(int key, CEditor *ce, VOIDPTR info);
typedef void change_proc_t(CEditor *ce, VOIDPTR info);
...
struct _CEditor
   ...
   BOOLEAN   done;
   BOOLEAN   hidden;
   other_key_proc_t *other_key;
   change_proc_t *change;
   void     *info;
...
/* public: */
   ...
   static CEditor *CEditor_Construct( int x, int y, char letter,
				      other_key_proc_t *other_key,
				      change_proc_t *change, VOIDPTR info);
   static void CEditor_Destroy   (CEditor *which);
   static void CEditor_Draw      (CEditor *which);
   static void CEditor_SetPos    (CEditor *which, int x, int y);
   static void CEditor_SetVal    (CEditor *which, int val);
   static int  CEditor_GetVal    (CEditor *which);
   static void CEditor_SetDone   (CEditor *which, BOOLEAN done);
   static void CEditor_SetHidden (CEditor *which, BOOLEAN hidden);
   static int  CEditor_Edit      (CEditor *which);
