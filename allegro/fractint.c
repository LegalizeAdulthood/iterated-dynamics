...
char file_name_stack[16][FILE_MAX_FNAME+FILE_MAX_EXT];
...
int brwscheckparms,brwschecktype;
char browsemask[13];
...
#ifdef HAVE_ALLEGRO_DRIVER
END_OF_MAIN();
  /* This is needed by the Allegro code. */
  /* Will also be needed by a Windows port. */
#endif

int check_key()
{
   int key;
   if((key = fkeypressed()) != 0) {
      if (show_orbit)
         scrub_orbit();
      if(key != 'o' && key != 'O') {
         fflush(stdout);
