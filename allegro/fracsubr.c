...
#define SLEEPINIT 250 /* milliseconds for calibration */
    ...
    if(scalems==0L) /* calibrate */
    {
        ...
        int i,elapsed;
        scalems = 1L;
        if(fkeypressed()) /* check at start, hope to get start of timeslice */
           goto sleepexit;
...
void w_snd(int tone)
{
   ...
   taborhelp = 0;
   if(!fkeypressed()) { /* fkeypressed calls mute() if TAB or F1 pressed */
...
/*   if(10 < tone && tone < 5000)  better limits? */
      if(driver_sound_on(tone)) {
         ...
         if(!taborhelp) /* kludge because wait_until() calls fkeypressed */
            driver_sound_off();
      }
   }
}
