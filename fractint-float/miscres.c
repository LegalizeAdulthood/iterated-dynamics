...
*/
   {
      char *which = "OTHER";
      sprintf(msg,"minstackavail %d bitshift %d fractype %d use_grid %d",
         minstackavail,bitshift,fractype,use_grid);

       if(floatbailout == (int (near *)(void))asmfpMODbailout)
          which = "MODASM";
       else if(floatbailout == (int (near *)(void))fpMODbailout)
          which = "MODC";
       else if(floatbailout == (int (near *)(void))asmfpREALbailout)
          which = "REALASM";
       sprintf(msg,"%s",which);
   }
   putstring(s_row++,2,C_GENERAL_HI,msg);
...
top:
   ...
   if (display3d > 0)
      ...
      putstring(s_row,16,C_GENERAL_HI,curfractalspecific->name);
      i = 0;
      if (fractype == FFORMULA)
      {
   ...
   if(bf_math==0)
   {
      ...
      putstring(-1,-1,C_GENERAL_HI, sflag_is_activated);                                             
      i = 1;
   } 
   else
   {
